...
def get_user_profile(user_id):
    ...
    ''', (user_id,))
    recently_played = [row[0] for row in cursor.fetchall()]
...
def add_track_to_playlist(playlist_id, track_id, user_id):
...
# -------------------------------
def toggle_favorite_track(user_id, track_id, favorite=True):
...
def get_playback_state(user_id):
    ...
    cursor.execute('''
    SELECT current_track, position_ms, is_playing, repeat_mode, shuffle, volume
    FROM playback_state
    WHERE user_id = ?
    ''', (user_id,))
    ...
    if not state:
        ...
        state = (None, 0, 0, 'off', 0, 100)
    
    # Get queue
    cursor.execute('''
    SELECT track_id FROM queue
    WHERE user_id = ?
    ORDER BY position
    ''', (user_id,))
    ...
    return {
        ...
        'position_ms': state[1],
        'is_playing': bool(state[2]),
        'repeat_mode': state[3],
        'shuffle': bool(state[4]),
        'volume': state[5],
        'queue': queue,
        'recently_played': recently_played
    }
...
def start_playback(user_id, track_id=None):
    ...
    if track_id:
        ...
        cursor.execute('''
        INSERT INTO recently_played (user_id, track_id)
        VALUES (?, ?)
        ''', (user_id, track_id))
...
def skip_to_next(user_id):
    ...
    start_playback(user_id, next_track)
...
def skip_to_previous(user_id):
    ...
    if state['current_track']:
        ...
        cursor.execute('''
        INSERT INTO queue (user_id, track_id, position)
        VALUES (?, ?, 0)
        ''', (user_id, state['current_track']))
        
        conn.commit()
        conn.close()
    
    # Start playing previous track
    start_playback(user_id, prev_track)
    
    return True, "Skipped to previous track"

def add_to_queue(user_id, track_id):
    ...
    INSERT INTO queue (user_id, track_id, position)
    VALUES (?, ?, ?)
    ''', (user_id, track_id, max_position + 1))
...
def get_track_details(track_id):
    ...
    url = f"{RECCOBEATS_BASE_URL}/track/{track_id}"
...
def get_playback_url(track_id):
    ...
    url = f"{RECCOBEATS_BASE_URL}/track/{track_id}/stream"
    ...
    try:
        ...
        print(f"Error getting playback URL: {e}")
...
def search_tracks(query, limit=20, offset=0):
    ...
    url = f"{RECCOBEATS_BASE_URL}/search"
...
def search_musicbrainz(query, entity_type="recording", limit=25, offset=0):
    ...
    headers = {
        "User-Agent": "MusicRecommendationApp/1.0.0 (contact@example.com)",
...
def music_player():
    ...
    if st.session_state.current_track:
        ...
        if track_details:
            ...
            with col1:
                st.image(track_details.get('album_art', 'https://via.placeholder.com/300'), width=100)
            
            with col2:
                st.markdown(f"<h4>{track_details.get('title')}</h4>", unsafe_allow_html=True)
                st.write(f"Artist: {track_details.get('artist')}")
                st.write(f"Album: {track_details.get('album')}")
            
            # If there's a stream URL, display audio player
            stream_url = get_playback_url(st.session_state.current_track)
            if stream_url:
    ...
    with cols[3]:
        if st.button("❤️", key="like"):
            if st.session_state.current_track:
                toggle_favorite_track(st.session_state.user_id, st.session_state.current_track, True)
...
def home_page():
    ...
    api_recommendations = get_track_recommendations(limit=5)
    ...
    if api_recommendations and 'tracks' in api_recommendations:
        st.header("Recommended for You")
        
        # Display recommendations in a grid
        cols = st.columns(5)
        for i, track in enumerate(api_recommendations.get('tracks', [])[:5]):
            with cols[i % 5]:
                st.markdown(f"""
                ...
                if st.button("Play", key=f"play_rec_{i}"):
                    start_playback(st.session_state.user_id, track.get('id'))
                    st.rerun()
        ...
        for i, (user_id, track_id) in enumerate(ml_recommendations):
            ...
            with col1:
                if st.button("Play Now", key=f"play_ml_{i}"):
                    start_playback(st.session_state.user_id, track_id)
                    st.rerun()
            with col2:
                if st.button("Add to Queue", key=f"queue_ml_{i}"):
                    add_to_queue(st.session_state.user_id, track_id)
    ...
    if recently_played:
        ...
        for i, track_id in enumerate(recently_played[:5]):
            track_details = get_track_details(track_id)
            ...
            with col1:
                st.markdown(f"""
                <div style="display: flex; align-items: center;">
                    <img src="{track_details.get('album_art', 'https://via.placeholder.com/300')}" style="width: 40px; height: 40px; object-fit: cover; margin-right: 10px; border-radius: 3px;">
                    <div>
                        <p style="margin: 0;"><b>{track_details.get('title')}</b> - {track_details.get('artist')}</p>
                    </div>
                </div>
                """, unsafe_allow_html=True)
            with col2:
...
def search_page():
    ...
    if search_query:
        # Search tracks using ReccoBeats API
        ...
        if results and 'tracks' in results and results['tracks']:
            ...
            # Fallback to MusicBrainz if ReccoBeats returns no results
            st.info("No results found in ReccoBeats. Searching MusicBrainz...")
            mb_results = search_musicbrainz(search_query)
            
            if mb_results and 'recordings' in mb_results:
                st.subheader("Tracks (from MusicBrainz)")
                
                for i, recording in enumerate(mb_results['recordings'][:10]):
                    artist_credit = recording.get('artist-credit', [{}])[0].get('name', 'Unknown Artist')
                    col1, col2 = st.columns([4, 1])
                    with col1:
                        st.write(f"{recording.get('title')} - {artist_credit}")
                    with col2:
                        if st.button("Details", key=f"details_mb_{i}"):
                            st.session_state.mb_recording = recording
            else:
...
def library_page():
                ...
                with col2:
                    if st.button("Play", key=f"play_playlist_{playlist['id']}"):
                        ...
                        if playlist_data and playlist_data['tracks']:
                            first_track = playlist_data['tracks'][0][0]  # First track ID
                            st.session_state.current_track = first_track
                            st.session_state.is_playing = True
                            start_playback(st.session_state.user_id, first_track)
                            
                            # Add remaining tracks to queue
                            for track_data in playlist_data['tracks'][1:]:
                                add_to_queue(st.session_state.user_id, track_data[0])
        else:
    ...
    with tab2:
        ...
        if favorites:
            for i, (track_id, added_at) in enumerate(favorites):
                track_details = get_track_details(track_id)
                ...
                if track_details:
                    ...
                    with col3:
                        if st.button("Remove", key=f"remove_fav_{i}"):
                            toggle_favorite_track(st.session_state.user_id, track_id, False)
                            st.rerun()
        else:
    ...
    with tab3:
        ...
        if queue:
            for i, track_id in enumerate(queue):
                track_details = get_track_details(track_id)
                ...
                if track_details:
                    ...
                    with col1:
                        ...
                        <div style="display: flex; align-items: center;">
                            <img src="{track_details.get('album_art', 'https://via.placeholder.com/300')}" style="width: 40px; height: 40px; object-fit: cover; margin-right: 10px; border-radius: 3px;">
                            <div>
                                <p style="margin: 0;">{i+1}. <b>{track_details.get('title')}</b> - {track_details.get('artist')}</p>
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
                    with col2:
                        if st.button("Remove", key=f"remove_queue_{i}"):
                            ...
                            ''', (st.session_state.user_id, track_id, i))
...
def playlist_page():
    ...
    if playlist_data['tracks']:
        for i, (track_id, added_at) in enumerate(playlist_data['tracks']):
            track_details = get_track_details(track_id)
    ...
    if playlist_data['is_owner']:
        ...
        if search_query:
            ...
            if search_results and 'tracks' in search_results and search_results['tracks']:
...
def profile_page():
    ...
    if st.session_state.ml_processed_data is not None:
        # Create listening activity chart
        if 'timestamp' in st.session_state.ml_processed_data.columns:
            # Extract hour from timestamp
            st.session_state.ml_processed_data['hour'] = pd.to_datetime(
                st.session_state.ml_processed_data['timestamp']).dt.hour
            
            # Count plays by hour
            hour_counts = st.session_state.ml_processed_data.groupby('hour').size().reset_index(name='count')
        ...
        # Genre distribution
        if 'track_genre' in st.session_state.ml_processed_data.columns:
            genre_counts = st.session_state.ml_processed_data['track_genre'].value_counts().head(5)
...
def analytics_page():
    ...
    if 'action' in st.session_state.ml_processed_data.columns:
    ...
    if st.session_state.user_data and 'tracks' in st.session_state.user_data:
...
def add_to_playlist_modal():
    if 'add_to_playlist' in st.session_state and st.session_state.add_to_playlist:
        ...
        if playlists:
            ...
            if st.button("Add"):
                success, message = add_track_to_playlist(playlist_id, st.session_state.add_track_id, st.session_state.user_id)
                if success:
                    st.success(message)
                else:
                    st.error(message)
                ...
                del st.session_state.add_track_id
                st.rerun()
            
            if st.button("Cancel"):
                st.session_state.add_to_playlist = False
                if 'add_track_id' in st.session_state:
                    del st.session_state.add_track_id
                st.rerun()
        else:
